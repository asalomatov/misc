...
do
    ...
    then
        ...
        if [[ $(vcfsamplenames /mnt/ceph/asalomatov/SSC_Eichler/rerun/ssc${i}/${i}-${caller}-vars-flr.vcf.gz | grep -m 1 '.s1') ]]; then 
            echo 'quad'
            vt normalize -q -r $GENOMEREF /mnt/ceph/asalomatov/SSC_Eichler/rerun/ssc${i}/${i}-${caller}-vars-flr.vcf.gz | $snpsift filter " (GEN[2].GT != '0|0' &  GEN[2].GT != '0/0') |( GEN[3].GT != '0|0' & GEN[3].GT != '0/0') " | grep -v '^#' | awk '{print $1"\t"$2"\t"$4"\t"$5}' > ${i}-${caller}-vars.txt
        else
            echo 'trio'
            vt normalize -q -r $GENOMEREF /mnt/ceph/asalomatov/SSC_Eichler/rerun/ssc${i}/${i}-${caller}-vars-flr.vcf.gz | $snpsift filter " GEN[2].GT != '0|0' &  GEN[2].GT != '0/0' " | grep -v '^#' | awk '{print $1"\t"$2"\t"$4"\t"$5}' > ${i}-${caller}-vars.txt
        fi
    else
        ...
        if [[ $(vcfsamplenames /mnt/ceph/asalomatov/SSC_Eichler/rerun/ssc${i}/${i}-${caller}-vars.vcf.gz | grep -m 1 '.s1') ]]; then 
            echo 'quad'
            vt normalize -q -r $GENOMEREF /mnt/ceph/asalomatov/SSC_Eichler/rerun/ssc${i}/${i}-${caller}-vars.vcf.gz | $snpsift filter " (GEN[2].GT != '0|0' &  GEN[2].GT != '0/0') |( GEN[3].GT != '0|0' & GEN[3].GT != '0/0') " | grep -v '^#' | awk '{print $1"\t"$2"\t"$4"\t"$5}' > ${i}-${caller}-vars.txt
        else
            echo 'trio'
            vt normalize -q -r $GENOMEREF /mnt/ceph/asalomatov/SSC_Eichler/rerun/ssc${i}/${i}-${caller}-vars.vcf.gz | $snpsift filter " GEN[2].GT != '0|0' &  GEN[2].GT != '0/0' " | grep -v '^#' | awk '{print $1"\t"$2"\t"$4"\t"$5}' > ${i}-${caller}-vars.txt
        fi
    fi
done
