...
do
    ...
    then
        ...
        if [[ $(vcfsamplenames /mnt/ceph/asalomatov/SSC_Eichler/rerun/ssc${i}/${i}-${caller}-vars-flr.vcf.gz | grep -m 1 '.s1') ]]; then 
            echo 'quad'
            vt normalize -q -r $GENOMEREF /mnt/ceph/asalomatov/SSC_Eichler/rerun/ssc${i}/${i}-${caller}-vars-flr.vcf.gz | $snpsift filter " (GEN[2].GT != '0|0' &  GEN[2].GT != '0/0') | ( GEN[3].GT != '0|0' & GEN[3].GT != '0/0') " | vcfintersect -b /mnt/scratch/asalomatov/data/SSCexome/b37.exome-pm50.bed | grep -v '^#' | awk '{print $1"\t"$2"\t"$4"\t"$5}' | awk '{ if (length($3) == length($4)) print $1"\t"$2"\tsub("$3"->"$4")"; if (length($3) < length($4)) print $1"\t"$2+1"\tins("substr($4,2)")"; if (length($3) > length($4)) print $1"\t"$2+1"\tdel("length($3)-length($4)")"; }' > ${i}-${caller}-vars.txt
        else
            echo 'trio'
            vt normalize -q -r $GENOMEREF /mnt/ceph/asalomatov/SSC_Eichler/rerun/ssc${i}/${i}-${caller}-vars-flr.vcf.gz | $snpsift filter " GEN[2].GT != '0|0' &  GEN[2].GT != '0/0' " | vcfintersect -b /mnt/scratch/asalomatov/data/SSCexome/b37.exome-pm50.bed | grep -v '^#' | awk '{print $1"\t"$2"\t"$4"\t"$5}' | awk '{ if (length($3) == length($4)) print $1"\t"$2"\tsub("$3"->"$4")"; if (length($3) < length($4)) print $1"\t"$2+1"\tins("substr($4,2)")"; if (length($3) > length($4)) print $1"\t"$2+1"\tdel("length($3)-length($4)")"; }'  > ${i}-${caller}-vars.txt
        fi
    else
        ...
        if [[ $(vcfsamplenames /mnt/ceph/asalomatov/SSC_Eichler/rerun/ssc${i}/${i}-${caller}-vars.vcf.gz | grep -m 1 '.s1') ]]; then 
            echo 'quad'
            vt normalize -q -r $GENOMEREF /mnt/ceph/asalomatov/SSC_Eichler/rerun/ssc${i}/${i}-${caller}-vars.vcf.gz | $snpsift filter " (GEN[2].GT != '0|0' &  GEN[2].GT != '0/0') | ( GEN[3].GT != '0|0' & GEN[3].GT != '0/0') " | vcfintersect -b /mnt/scratch/asalomatov/data/SSCexome/b37.exome-pm50.bed  | grep -v '^#' | awk '{print $1"\t"$2"\t"$4"\t"$5}'  | awk '{ if (length($3) == length($4)) print $1"\t"$2"\tsub("$3"->"$4")"; if (length($3) < length($4)) print $1"\t"$2+1"\tins("substr($4,2)")"; if (length($3) > length($4)) print $1"\t"$2+1"\tdel("length($3)-length($4)")"; }' > ${i}-${caller}-vars.txt
        else
            echo 'trio'
            vt normalize -q -r $GENOMEREF /mnt/ceph/asalomatov/SSC_Eichler/rerun/ssc${i}/${i}-${caller}-vars.vcf.gz | $snpsift filter " GEN[2].GT != '0|0' &  GEN[2].GT != '0/0' " | vcfintersect -b /mnt/scratch/asalomatov/data/SSCexome/b37.exome-pm50.bed | grep -v '^#' | awk '{print $1"\t"$2"\t"$4"\t"$5}' | awk '{ if (length($3) == length($4)) print $1"\t"$2"\tsub("$3"->"$4")"; if (length($3) < length($4)) print $1"\t"$2+1"\tins("substr($4,2)")"; if (length($3) > length($4)) print $1"\t"$2+1"\tdel("length($3)-length($4)")"; }'  > ${i}-${caller}-vars.txt
        fi
    fi
done
