...
for smpl in df_descr['smpl_id']:
    ...
    df_exome['perc'] = df_exome['perc'].astype(float)
    print df_exome.head()
...
nyg.gt_p1.value_counts()
#nyg = nyg[~nyg['gt_p1'].isin(['0/0'])]
...
for f in ['1-blood', '2-blood', '3-blood', '4-blood', '1-saliva', '2-saliva', '3-saliva', '4-saliva']:
    ...
    print fname
    x = pd.read_csv(fname, sep="\t", header=None)
    x.columns = ['chr', 'pos', 'id', 'ref', 'alt', 'qual', 'filt', 'info', 'fmt', 'gt_p1_full', 'gt_fa_full',
            'gt_mo_full']
    x.head()
    x['gt_p1'] = x.gt_p1_full.apply(lambda x: x[:3])
    x['gt_fa'] = x.gt_fa_full.apply(lambda x: x[:3])
    x['gt_mo'] = x.gt_mo_full.apply(lambda x: x[:3])
    x['ac_p1'] = x.gt_p1_full.apply(lambda x: int(x.split(':')[1]))
    x['ac_fa'] = x.gt_fa_full.apply(lambda x: int(x.split(':')[1]))
    x['ac_mo'] = x.gt_mo_full.apply(lambda x: int(x.split(':')[1]))
    x['rc_p1'] = x.gt_p1_full.apply(lambda x: int(x.split(':')[2]))
    x['rc_fa'] = x.gt_fa_full.apply(lambda x: int(x.split(':')[2]))
    x['rc_mo'] = x.gt_mo_full.apply(lambda x: int(x.split(':')[2]))
    x['descr'] = f
    x['fam'] = fam
    x['protocol'] = prot
    all_list.append(x)
...
baylor = baylor[~baylor.apply(lambda x: "." in x['gt'], axis=1)]
baylor.gt_p1.value_counts()
#baylor = baylor[~baylor['gt_p1'].isin(['0/0'])]
...
sf_baylor_bam['gt'].value_counts()
sf_baylor_bam = sf_baylor_bam[~sf_baylor_bam['gt'].isin(['0/0_0/0_0/0'])]
#sf_baylor_bam = sf_baylor_bam[~sf_baylor_bam['gt_p1'].isin(['0/0'])]
sf_baylor_bam = sf_baylor_bam[sf_baylor_bam['filt'].isin(['PASS'])]
...
sf_nyg_bam['gt'].value_counts()
sf_nyg_bam = sf_nyg_bam[~sf_nyg_bam['gt'].isin(['0/0_0/0_0/0'])]
#sf_nyg_bam = sf_nyg_bam[~sf_nyg_bam['gt_p1'].isin(['0/0'])]
sf_nyg_bam = sf_nyg_bam[sf_nyg_bam['filt'].isin(['PASS'])]
sf_nyg_bam['gt_p1'].value_counts()
...
def varsumm(x):
    ...
    res = [round(x,3) for x in [var_pos, var_pos_al, var_pos_al_gt]]
...
def varsumm_detail(x, extra_args):
    label1 = extra_args[0]
    label2 = extra_args[1]
    field = extra_args[2]
    Nround = extra_args[3]
    c1 = x[field]==label1
    c2 = x[field]==label2
    set_1_pos = set(x['pos_var'][c1])
    set_2_pos = set(x['pos_var'][c2])
    set_1_al = set(x['pos_allele_var'][c1])
    set_2_al = set(x['pos_allele_var'][c2])
    set_1_al_gt = set(x['pos_allele_gt_var'][c1])
    set_2_al_gt = set(x['pos_allele_gt_var'][c2])
    var_pos = float(len(set_1_pos.intersection(set_2_pos)))/len(set_1_pos.union(set_2_pos))
    var_pos_1 = float(len(set_1_pos.intersection(set_2_pos)))/len(set_1_pos)
    var_pos_2 = float(len(set_1_pos.intersection(set_2_pos)))/len(set_2_pos)
    var_pos_al = float(len(set_1_al.intersection(set_2_al)))/len(set_1_al.union(set_2_al))
    var_pos_al_1 = float(len(set_1_al.intersection(set_2_al)))/len(set_1_al)
    var_pos_al_2 = float(len(set_1_al.intersection(set_2_al)))/len(set_2_al)
    var_pos_al_gt = float(len(set_1_al_gt.intersection(set_2_al_gt)))/len(set_1_al_gt.union(set_2_al_gt))
    var_pos_al_gt_1 = float(len(set_1_al_gt.intersection(set_2_al_gt)))/len(set_1_al_gt)
    var_pos_al_gt_2 = float(len(set_1_al_gt.intersection(set_2_al_gt)))/len(set_2_al_gt)
    var_pos_N = len(set_1_pos.union(set_2_pos))
    var_pos_al_N = len(set_1_al.union(set_2_al))
    var_pos_al_gt_N = len(set_1_al_gt.union(set_2_al_gt))
    res = [str(round(x,Nround)) for x in [var_pos_1,var_pos,var_pos_2,var_pos_al_1,var_pos_al,var_pos_al_2,var_pos_al_gt_1,var_pos_al_gt,var_pos_al_gt_2,]]
    res0 = map(lambda x: myfunc(x,Nround+2), res)
    res1 = ['/'.join(res0[:3]), '/'.join(res0[3:6]),'/'.join(res0[6:])]
    res2 = res1 + [var_pos_N,var_pos_al_N,var_pos_al_gt_N]
    #res2 = res1 + [var_pos_N]
    return pd.Series (res2, index = ['/'.join([label1,'var_pos',label2]),'/'.join([label1,'var_pos_al',label2]),
        '/'.join([label1,'var_pos_al_gt',label2]), 'var_pos_N', 'var_pos_al_N', 'var_pos_al_gt_N']) 

def varsumm_detail_num(x, extra_args):
    label1 = extra_args[0]
    label2 = extra_args[1]
    field = extra_args[2]
    field_var = extra_args[3]
    c1 = x[field]==label1
    c2 = x[field]==label2
    set_1 = set(x[field_var][c1])
    set_2 = set(x[field_var][c2])
    set_intersect = len(set_1.intersection(set_2))
    set_1_only = len(set_1.difference(set_2))
    set_2_only = len(set_2.difference(set_1))
    res = [set_intersect,set_1_only,set_2_only]
    return pd.Series (res, index = ['common', 'blood', 'saliva'])

def varsumm_detail_perc(x, extra_args):
    label1 = extra_args[0]
    label2 = extra_args[1]
    field = extra_args[2]
    field_var = extra_args[3]
    ...
    set_1 = set(x[field_var][c1])
    set_2 = set(x[field_var][c2])
    set_intersect = float(len(set_1.intersection(set_2)))/len(set_1.union(set_2))
    set_1_only = float(len(set_1.difference(set_2)))/len(set_1.union(set_2))
    set_2_only = float(len(set_2.difference(set_1)))/len(set_1.union(set_2))
    res = [set_intersect,set_1_only,set_2_only]
    return pd.Series (res, index = ['common', 'blood', 'saliva'])
...
#df_all = pd.concat([baylor, sf_baylor_bam])
df_all.source.value_counts()
df_all.protocol.value_counts()
df_all.fam.value_counts()
...
df2
pl1 = df2.plot(kind='bar', stacked=True, fontsize=20)
plt.show()
pl1.get_figure().savefig('/mnt/xfs1/home/asalomatov/dec16_barchart.pdf')


df_all.groupby(['source','fam']).apply(varsumm_detail_num, ('blood', 'saliva', 'protocol','pos_var'))
df_all.groupby(['vartype','source','fam']).apply(varsumm_detail_num, ('blood', 'saliva', 'protocol','pos_allele_var'))
df_var = df_all[df_all.source.isin(['sf_baylor_bam'])].groupby(['vartype','fam']).apply(varsumm_detail_num, ('blood', 'saliva', 'protocol','pos_allele_gt_var'))
df_v = df_all[df_all.source.isin(['sf_baylor_bam'])].groupby(['source','fam']).apply(varsumm_detail_num, ('blood', 'saliva', 'protocol','pos_allele_gt_var'))
df_var_perc = df_all[df_all.source.isin(['sf_baylor_bam'])].groupby(['vartype','fam']).apply(varsumm_detail_perc, ('blood', 'saliva', 'protocol','pos_allele_gt_var'))
df_v_perc = df_all[df_all.source.isin(['sf_baylor_bam'])].groupby(['source','fam']).apply(varsumm_detail_perc, ('blood', 'saliva', 'protocol','pos_allele_gt_var'))
df_var.columns
df_var.index
df_var.query('vartype == "snp"')

fig, axes = plt.subplots(nrows=1, ncols=3)

share_Y=True
df_v_perc.plot(ax=axes[0], kind='bar', sharey=share_Y, stacked=True, fontsize=16, legend=False, title='Concordance of All Variants\n Chrom-Pos-Alleles-Family Genotype')
df_var_perc.query('vartype == "snp"').plot(ax=axes[1], kind='bar', sharey=share_Y, stacked=True, fontsize=16, legend=False, title='Concordance of SNPs\n Chrom-Pos-Alleles-Family Genotype')
df_var_perc.query('vartype == "indel"').plot(ax=axes[2], kind='bar', sharey=share_Y, stacked=True, fontsize=16, legend=False, title='Concordance of INDELs\n Chrom-Pos-Alleles-Family Genotype')
axes[0].set_title(axes[0].get_title(), fontsize=20)
axes[0].set_xlabel(axes[0].get_xlabel(), fontsize=20)
patches, labs = axes[0].get_legend_handles_labels()
axes[0].legend(patches, labs, loc=4)
axes[1].set_title(axes[1].get_title(), fontsize=20)
axes[1].set_xlabel(axes[1].get_xlabel(), fontsize=20)
patches, labs = axes[1].get_legend_handles_labels()
axes[1].legend(patches, labs, loc=4)
axes[2].set_title(axes[2].get_title(), fontsize=20)
axes[2].set_xlabel(axes[2].get_xlabel(), fontsize=20)
patches, labs = axes[2].get_legend_handles_labels()
axes[2].legend(patches, labs, loc=4)
for x in axes[0].containers:
    plt.setp(x, width=.7)
for x in axes[1].containers:
    plt.setp(x, width=.7)
for x in axes[2].containers:
    plt.setp(x, width=.7)
fig.tight_layout()
fig.subplots_adjust(wspace=0.2)
#plt.subplots_adjust(hspace=.5)
fig.savefig('/mnt/xfs1/home/asalomatov/dec16_BLvsSA_vartype_perc.png', format='png')
fig.show()

# each plot separately
#pl1 = df_var.query('vartype == "snp"').plot(kind='bar', stacked=True, fontsize=16, title='Concordance of SNPs\n Chrom-Pos-Alleles-Family Genotype')
#pl1 = df_var.query('vartype == "indel"').plot(kind='bar', stacked=True, fontsize=16, title='Concordance of INDELs\n Chrom-Pos-Alleles-Family Genotype')
pl1 = df_v.plot(kind='bar', stacked=True, fontsize=16, title='Concordance of All Variants\n Chrom-Pos-Alleles-Family Genotype')
pl1.set_title(pl1.get_title(), fontsize=25)
pl1.set_xlabel(pl1.get_xlabel(), fontsize=20)
for x in pl1.containers:
    plt.setp(x, width=.7)
plt.tight_layout()
pl1.get_figure().savefig('/mnt/xfs1/home/asalomatov/dec16_BLvsSA_all.png', format='png')
plt.show()


df_allCall.groupby(['source','vartype']).apply(len)

df.groupby(['source','fam']).apply(varsumm)
df_all.groupby(['source','fam']).apply(varsumm)
df_all.groupby(['source','fam']).apply(varsumm_detail, ('blood', 'saliva', 'protocol', 3)).to_excel('/mnt/xfs1/home/asalomatov/dec16_blood_vs_saliva_detail.xls')
df_all.groupby(['vartype','source','fam']).apply(varsumm_detail, ('blood', 'saliva', 'protocol', 3)).to_excel('/mnt/xfs1/home/asalomatov/dec16_blood_vs_saliva_vartype_detail.xls')

df_all.groupby(['vartype','source','fam']).apply(varsumm)
df_all.groupby(['source','fam']).apply(varsumm).to_excel('/mnt/xfs1/home/asalomatov/dec14_blood_vs_saliva.xls')
df_all.groupby(['vartype','source','fam']).apply(varsumm).to_excel('/mnt/xfs1/home/asalomatov/dec14_blood_vs_saliva_vartype.xls')
...
df_all.groupby(['protocol', 'fam']).apply(protsumm, ('sf_baylor_bam', 'baylor')).to_excel('/mnt/xfs1/home/asalomatov/nov24_baylor_vs_sfHC.xls') 
...
df_allCall.groupby(['protocol', 'fam', 'vartype']).apply(protsumm, ('sf_baylor_bam',
    'baylor')).to_excel('/mnt/xfs1/home/asalomatov/nov12_baylor_vs_sfHCFBPL_vartype.xls') 
