...
#make -j $P -f /mnt/xfs1/scratch/asalomatov/BloodVsSaliva/scripts/filterBamTarget.mk TARGBED=/mnt/scratch/asalomatov/data/b37/b37.exome-pm50.bed NEWSUFFIX=-exome INCLMK=$inclmk PREFIX=$famcode INDIR=$inpd OUTDIR=$outdir LOGDIR=$outdir
#ret=$?
#echo $ret
#if [ $ret -ne 0 ]; then
...
make -j $P -f /mnt/xfs1/scratch/asalomatov/BloodVsSaliva/scripts/filterBamTarget.mk TARGBED=/mnt/scratch/asalomatov/data/b37/ensembl-genes-coding_all_exome.bed NEWSUFFIX=-genes INCLMK=$inclmk PREFIX=$famcode INDIR=$inpd OUTDIR=$outdir LOGDIR=$outdir
ret=$?
echo $ret
if [ $ret -ne 0 ]; then
    echo "filterBamTarget.mk INCLMK=$inclmk finished with errors"
